...
import { init, doctor } from "../dist/index.js";
...
program
  ...
  .addOption(
    new Option("--preset <preset>", "预设方案")
      .choices(["minimal", "standard", "full"])
      .default("standard"),
  )
  .addOption(
    new Option("--framework <framework>", "项目框架")
  ...
  .option("--jsdoc", "启用 JSDoc（生成相关配置）")
  .option("--no-jsdoc", "不启用 JSDoc")
  .action(async (options) => {
    try {
      ...
      const detail =
        error?.stderr || error?.shortMessage || error?.message || error;
      console.error(
